...
fitmc@pars@qmodel[[6]]@vcov[] <- cov(t(coef(qmodel(pars(fit))[[6]])@.Data))
...
fitmc@pars@vmodel[[7]]@vcov[] <- cov(t(coef(vmodel(pars(fit))[[7]])@.Data))
mus <- median(ssb(stk)[,"2018"])
muf <- median(fbar(stk)[,"2018"])
...
# NOTE: 0.1 has the best compromise between CI coverage for both f 
...
## best for constM final mcprob = 0.2, accrate= 0.34
save(aa.stk,fit50,fit40,fit30,fit20,fit10,fit00,mcmctest.df, 
     file = "~/pCloudDrive/ICCAT/runs/a4a/MCMC/final_constM_disc.RData")
