...
author: "D. Mantopoulou-Palouka^[Joint Research Center. Corresponding author: Danai.MANTOPOULOU-PALOUKA@ec.europa.eu], G. Tserpes^[Hellenic Center for Marine Research, Heraklion, Greece], A. Kimoto^[International Commission for the Conservation of Atlantic Tunas, Madrid Spain], E. Jardim^[Portuguese Institute for Sea and Atmosphere, Lisbon, Portugal]"
date: ''
output:
...
suppressMessages({
  ...
  library(gridExtra)
  library(patchwork)
  library(flextable) # Recommended for Word tables
})

# Remove grids to comply with ICCAT figure guidelines
theme_set(
...
The observation variance ($\sigma^2$) of the catches was modeled using a spline across ages ($\sim s(\text{age}, k = 3)$) assuming no year effect, while the observation variances ($\tau^2$) for all tuning indices were assumed constant ($\sim 1$). 

The population's initial age structure in the first year of the assessment ($N_1$) was estimated using a simple age smoother ($\sim s(\text{age}, k = 3)$).
...
ggplot(subset(res_df_l, qname == "catch.n"), aes(x = year, y = data)) +
  ...
  theme(legend.position = "none") + ylim(-3,3)
```
...
```{r retro-85-l, fig.height=4.1, fig.cap="Retrospective analysis (5 peels) for the Lorenzen M candidate run."}
...
ref_table <- data.frame(
  Metric = c("FMSY", "BMSY (t)", "F2024", "B2024 (t)", "F2024 / FMSY", "B2024 / BMSY", "MSY (t)"),
...
ref_table_ft <- flextable(ref_table) %>%
  set_caption("Table 1: Current stock status and MSY-based reference points for the candidate runs.") %>%
  font(fontname = "Cambria", part = "all") %>%
  fontsize(size = 10, part = "all") %>%
  border_remove() %>%                                      # Removes gridlines
  hline_top(border = fp_border_default(width = 1.5)) %>%   # Standard top horizontal border
  hline_bottom(border = fp_border_default(width = 1.5)) %>%# Standard bottom border
...
p1 <- plot_kobe(a4a85_l, rp_85l, use_ssb = FALSE, title = "Lorenzen M")   
